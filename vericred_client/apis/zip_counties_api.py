...
class ZipCountiesApi(object):
    ...
    def zip_counties_get(self, zip_prefix, **kwargs):
        """
...
zip code and return both the selected zip and fips codes.
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.zip_counties_get(zip_prefix, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method zip_counties_get" % key
                )
            params[key] = val
        del params['kwargs']
        ...
        if ('zip_prefix' not in params) or (params['zip_prefix'] is None):
            raise ValueError("Missing the required parameter `zip_prefix` when calling `zip_counties_get`")
