...
# **plans_find_post**
> list[Plan] plans_find_post(query)
...
query = vericred_client.Query() # Query | Plan query

try: 
    # Find a set of plans for a Zip Code and County
    api_response = api_instance.plans_find_post(query)
    pprint(api_response)
except ApiException as e:
    print "Exception when calling PlansApi->plans_find_post: %s\n" % e
```
